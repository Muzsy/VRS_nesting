...
fn lv8_part_analysis_layer_tags_and_scores_are_sound() {
    ...
    let analyses = build_part_analyses_for_parts(&parts, SHEET_W, SHEET_H, SPACING_MM)
        .expect("part analyses");
    assert_eq!(analyses.len(), 3, "one analysis per unique part type");

    let lv8 = analyses.iter().find(|a| a.part_id.starts_with("Lv8")).expect("lv8 analysis");
    let tiny = analyses.iter().find(|a| a.part_id == "tiny_filler").expect("tiny analysis");

    // LV8: large / critical / edge-alignable; reuses profile values; hole-free.
    assert!(lv8.has_tag(ShapeTag::LargeAnchor), "LV8 must be large_anchor");
    assert!(lv8.has_tag(ShapeTag::EdgeAlignable), "LV8 must be edge_alignable");
    assert_eq!(lv8.diagnostics.criticality_tier, "critical");
    assert!(lv8.hole_free_solver_input, "LV8 solver input must be hole-free");
    assert!(lv8.shape_profile.quantity == 6, "profile values are reused verbatim");
    assert!(lv8.fit_difficulty.score >= 0.0 && lv8.fit_difficulty.score <= 1.0);

    // Tiny filler must never be an anchor.
    assert!(tiny.has_tag(ShapeTag::TinyFiller), "tiny part must be tiny_filler");
    assert!(!tiny.has_tag(ShapeTag::LargeAnchor), "tiny filler must not be large_anchor");
    assert!(!tiny.has_tag(ShapeTag::CriticalLarge));

    // Determinism: rebuild and compare LV8 fit-difficulty.
    let again = build_part_analyses_for_parts(&parts, SHEET_W, SHEET_H, SPACING_MM)
        .expect("rebuild");
    let lv8b = again.iter().find(|a| a.part_id.starts_with("Lv8")).unwrap();
    assert_eq!(lv8.fit_difficulty.score, lv8b.fit_difficulty.score, "fit-difficulty deterministic");
