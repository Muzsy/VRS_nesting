    ...
    pub(crate) fn report(&mut self, cand: ScoredPlacement, diag: &mut SparrowDiagnostics) -> bool {
        ...
        if let Some(idx) = self.samples.iter().position(|s| {
            ...
            if cand.eval() < self.samples[idx].eval() {
                ...
                if scope_active {
                    p.best_samples_dedup_rejects += 1;
                }
                t.add_to(&mut p.best_samples_insert_dedup_ms);
                return false;
            }
