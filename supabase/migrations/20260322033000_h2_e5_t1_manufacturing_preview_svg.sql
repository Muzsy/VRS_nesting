-- H2-E5-T1: manufacturing preview SVG — artifact kind + bridge update.
-- Scope: introduce manufacturing_preview_svg artifact kind and update
...
-- Does NOT create postprocessor/export/viewer-redesign scope.

-- 1) Add manufacturing_preview_svg to artifact_kind enum --------------------
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'app'
      ...
      AND e.enumlabel = 'manufacturing_preview_svg'
  ) THEN
    ALTER TYPE app.artifact_kind ADD VALUE 'manufacturing_preview_svg';
  END IF;
END
$$;
...
-- when present, and otherwise uses kind::text. For manufacturing_preview_svg
