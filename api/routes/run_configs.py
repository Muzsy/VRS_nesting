...
class RunConfigPartEntry(BaseModel):
...
def create_run_config(
    ...
    payload = {
        ...
        "stock_file_id": req.stock_file_id,
        "parts_config": [entry.model_dump() for entry in req.parts_config],
    }

    try:
        row = supabase.insert_row(table="run_configs", access_token=user.access_token, payload=payload)
    except SupabaseHTTPError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"create run-config failed: {exc}") from exc
...
def list_run_configs(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"list run-configs failed: {exc}") from exc
