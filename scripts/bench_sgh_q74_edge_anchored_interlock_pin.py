#!/usr/bin/env python3
"""SGH-Q74 - Big-part pitch-minimizing interlock row-seed benchmark.
...
Q74 = ROOT / "artifacts" / "benchmarks" / "sgh_q74"
INPUTS = Q74 / "inputs"
OUTPUTS = Q74 / "outputs"
LOGS = Q74 / "logs"
RUN_ID = "q74_A_edge_interlock_pin_2sheet_sp5"
...
Q74_GATES = {
    ...
    # SGH-Q74 opt-in edge-anchored interlock + pin (default OFF in production).
    "VRS_EDGE_INTERLOCK_SEED": "1",
}
GATE_KEYS = list(Q74_GATES) + ["VRS_SHEET_BUILDER_STRICT_LATEST", "VRS_BIG_ROW_SEED"]
...
def build_input(time_limit_s: int) -> Path:
    inp = {
        "contract_version": "v1",
        "project_name": "sgh_q74_big_part_interlock_rowseed",
        "seed": 42,
    ...
    path = INPUTS / f"q74_full276_2x1500x3000_margin5_spacing5_continuous_{time_limit_s}.json"
...
def run(inp: Path, time_limit_s: int) -> dict[str, Any]:
    ...
    (LOGS / f"{RUN_ID}.log").write_text(
        ...
        f"env={json.dumps(Q74_GATES, sort_keys=True)}\nstderr:\n{proc.stderr[:12000]}\n"
    )
...
def summarize(out: dict[str, Any], parts_by_id: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "q74_edge_seed_used": bpp.get("bpp_q74_edge_interlock_seed_used"),
        "q74_edge_part_id": bpp.get("bpp_q74_edge_interlock_part_id"),
        "q74_edge_rotation_deg": bpp.get("bpp_q74_edge_interlock_rotation_deg"),
        "q74_edge_locked_count": bpp.get("bpp_q74_edge_interlock_locked_count"),
        "q74_edge_copies_per_sheet": bpp.get("bpp_q74_edge_interlock_copies_per_sheet"),
        "q74_edge_seeded_count": bpp.get("bpp_q74_edge_interlock_seeded_count"),
...
def write_report(summary: dict[str, Any], input_rel: str) -> None:
    ...
    lines = [
        ...
        "| check | value |",
        "| --- | --- |",
        f"| edge interlock seed used | {str(bool(r['q74_edge_seed_used'])).lower()} |",
        f"| chosen rotation (deg) | {r['q74_edge_rotation_deg']} |",
        f"| pinned (locked) count | {r['q74_edge_locked_count']} |",
        f"| copies per sheet | {r['q74_edge_copies_per_sheet']} |",
        f"| seeded count | {r['q74_edge_seeded_count']} |",
    ...
    lines += [
        ...
        f"| **Q74 row-seed** | **{r['placed_count']}** | {r['used_sheets']} | {r['utilization_pct']} |",
        "",
        "## Visual Audit",
        "",
        ...
        f"- input: `artifacts/benchmarks/sgh_q74/{input_rel}`",
        f"- output: `artifacts/benchmarks/sgh_q74/outputs/{RUN_ID}_output.json`",
        f"- log: `artifacts/benchmarks/sgh_q74/logs/{RUN_ID}.log`",
    ]
    (Q74 / "q74_report.md").write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    input_rel = str(inp.relative_to(Q74))
    ...
    render_run(74, RUN_ID, input_rel, f"outputs/{RUN_ID}_output.json")
    render_manifest = json.loads((Q74 / "renders" / RUN_ID / "render_manifest.json").read_text())
    ...
    acceptance = {
        ...
        "edge_seed_used": bool(run_summary["q74_edge_seed_used"]),
    ...
    summary = {
        "task": "sgh_q74_big_part_interlock_rowseed",
    ...
    summary["finding"] = (
        "Q74 row-seeds the dominant repeated big type at the tightest CDE-clear (often non-orthogonal) "
    ...
    Q74.mkdir(parents=True, exist_ok=True)
    (Q74 / "q74_summary.json").write_text(json.dumps(summary, indent=2))
