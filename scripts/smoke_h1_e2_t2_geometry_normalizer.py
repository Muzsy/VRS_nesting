#!/usr/bin/env python3
"""H1-E2-T2 smoke: deterministic normalized geometry revisions."""
...
def _make_valid_dxf_bytes() -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix="vrs_h1e2t2_ok_") as tmp:
        ...
        msp.add_lwpolyline([(70.0, 10.0), (95.0, 10.0), (95.0, 35.0), (70.0, 35.0)], dxfattribs={"layer": "CUT_INNER", "closed": True})
        msp.add_lwpolyline([(10.0, 10.0), (40.0, 10.0), (40.0, 30.0), (10.0, 30.0)], dxfattribs={"layer": "CUT_INNER", "closed": True})
        doc.saveas(tmp_path)
...
def _assert_ring_list(name: str, value: Any) -> list[list[float]]:
    if not isinstance(value, list) or len(value) < 3:
        raise RuntimeError(f"{name} must be a non-empty ring list")
    out: list[list[float]] = []
    for idx, point in enumerate(value):
        if not isinstance(point, list) or len(point) != 2:
            raise RuntimeError(f"{name}[{idx}] must be [x, y]")
        x, y = point
        if not isinstance(x, (int, float)) or not isinstance(y, (int, float)):
            raise RuntimeError(f"{name}[{idx}] coordinates must be numeric")
...
def main() -> int:
    ...
    try:
        ...
        if ok_revision.get("canonical_format_version") != "normalized_geometry.v1":
            raise RuntimeError("canonical_format_version must be normalized_geometry.v1")
        ...
        if canonical_geometry.get("geometry_role") != "part":
            raise RuntimeError("canonical_geometry_jsonb.geometry_role mismatch")
        if canonical_geometry.get("format_version") != "normalized_geometry.v1":
            raise RuntimeError("canonical_geometry_jsonb.format_version mismatch")
        if canonical_geometry.get("units") != "mm":
            raise RuntimeError("canonical_geometry_jsonb.units mismatch")

        outer_ring = _assert_ring_list("outer_ring", canonical_geometry.get("outer_ring"))
        hole_rings_any = canonical_geometry.get("hole_rings")
        if not isinstance(hole_rings_any, list):
            raise RuntimeError("hole_rings must be list")
        hole_rings = [_assert_ring_list(f"hole_rings[{idx}]", ring) for idx, ring in enumerate(hole_rings_any)]

        if len(hole_rings) != 2:
            raise RuntimeError("expected exactly 2 hole rings in normalized payload")
        if hole_rings[0][0][0] > hole_rings[1][0][0]:
            raise RuntimeError("hole ring ordering is not deterministic")

        normalizer_meta = canonical_geometry.get("normalizer_meta")
        if not isinstance(normalizer_meta, dict):
            raise RuntimeError("normalizer_meta missing")
        if normalizer_meta.get("normalizer_version") != "normalized_geometry.v1":
            raise RuntimeError("normalizer_meta.normalizer_version mismatch")
        ring_policy = normalizer_meta.get("ring_policy")
        if not isinstance(ring_policy, dict):
            raise RuntimeError("normalizer_meta.ring_policy missing")
        if ring_policy.get("outer_orientation") != "ccw" or ring_policy.get("hole_orientation") != "cw":
            raise RuntimeError("normalizer_meta.ring_policy orientation mismatch")

        source_lineage = canonical_geometry.get("source_lineage")
        if not isinstance(source_lineage, dict):
            raise RuntimeError("source_lineage missing")
        if source_lineage.get("storage_path") != ok_storage_path:
            raise RuntimeError("source_lineage.storage_path mismatch")

        payload_bbox = canonical_geometry.get("bbox")
        if not isinstance(payload_bbox, dict):
            raise RuntimeError("canonical payload bbox missing")
        row_bbox = ok_revision.get("bbox_jsonb")
        if payload_bbox != row_bbox:
            raise RuntimeError("bbox_jsonb and payload bbox mismatch")

        if ok_revision.get("source_hash_sha256") != ok_file_row.get("sha256"):
            raise RuntimeError("source_hash_sha256 mismatch")

        expected_hash = _canonical_hash(canonical_geometry)
        if ok_revision.get("canonical_hash_sha256") != expected_hash:
            raise RuntimeError("canonical_hash_sha256 mismatch")
        ...
        if second_revision.get("canonical_geometry_jsonb") != canonical_geometry:
            raise RuntimeError("second normalized canonical payload mismatch")
        if second_revision.get("canonical_hash_sha256") != ok_revision.get("canonical_hash_sha256"):
            raise RuntimeError("second normalized canonical hash mismatch")
        ...
        if bad_revisions:
            raise RuntimeError("invalid DXF should not create normalized geometry revision")
        ...
        if missing_revisions:
            raise RuntimeError("missing object should not create normalized geometry revision")

        print("[PASS] H1-E2-T2 geometry normalizer smoke passed")
        return 0
    finally:
