#!/usr/bin/env python3
"""SGH-Q23R2 real Sparrow cutover completion benchmark.
...
STEM = "sgh_q23r2_real_sparrow_cutover_measurements"
...
def emit_md(results: dict) -> str:
    L = []
    L.append("# SGH-Q23R2 real Sparrow cutover completion — benchmark measurements\n")
    ...
    L.append("| fixture | pipeline | backend | seed | status | placed/req | runtime_ms | conv | iters | "
             "moves a/att | pairs i→f | raw i→f | cde_q | engine_builds | bphase_pruned | bbox_fb | lbf_fb |")
    L.append("|---|---|---|---:|---|---|---:|---|---:|---|---|---|---:|---:|---:|---|---:|---:|---:|---:|---:|---:|")
    for row in results["rows"]:
        ...
        L.append(
            ...
            f"{render_value(m.get('cde_total_queries'))} | {render_value(m.get('cde_engine_builds'))} | "
            f"{render_value(m.get('cde_broadphase_pruned'))} | "
            f"{render_value(m.get('cde_engine_builds'))} | {render_value(m.get('cde_batch_engine_builds'))} | "
            f"{render_value(m.get('total_engine_builds'))} | {render_value(m.get('cde_batch_candidate_queries'))} | "
