...
def main() -> int:
    ...
    try:
        ...
        if len(derivatives) < 2:
            raise RuntimeError(f"expected at least 2 derivatives for validated geometry, got {len(derivatives)}")

        by_kind = {str(row.get("derivative_kind")): row for row in derivatives}
        h1_required_kinds = {"nesting_canonical", "viewer_outline"}
        if not h1_required_kinds.issubset(set(by_kind.keys())):
            raise RuntimeError(f"missing required H1 derivative kinds: {sorted(h1_required_kinds - set(by_kind.keys()))}")
        ...
        if not isinstance(generated, dict) or not h1_required_kinds.issubset(set(generated.keys())):
            raise RuntimeError("regenerate result missing required H1 kinds")
        ...
        if len(derivatives_after) < 2:
            raise RuntimeError("regenerate should not lose derivative rows")
        by_kind_after = {str(row.get("derivative_kind")): row for row in derivatives_after}
        for kind in h1_required_kinds:
