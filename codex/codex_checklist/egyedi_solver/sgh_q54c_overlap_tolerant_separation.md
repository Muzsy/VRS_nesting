...
- [ ] Repo szabályfájlok + Q53 audit (refine not-clear-feladás) + Q52 rotation-correct állapot rögzítve.
- [ ] Minden módosított/létrehozott fájl szerepelt a YAML outputs listájában (scope-fegyelem).
- [ ] CDE final validation semantics nem gyengült; acceptance csak final_validation_tracker feasible.
- [ ] Nincs NFP, nincs bbox collision shortcut, nincs cavity/hole fősolver logika.
- [ ] Continuous rotation guardrail nem sérült (rotation-set a density_rotation_candidates-ből; nincs snap).
- [ ] Nincs part-id hack, nincs hardcoded 3+3.
- [ ] Task-specifikus unit tesztek elkészültek.
- [ ] Diagnosztika/report bizonyítja a tényleges separationt (iterations, overlap, rotation, fail_reason).
- [ ] `./scripts/verify.sh --report codex/reports/egyedi_solver/sgh_q54c_overlap_tolerant_separation.md` lefutott.
- [ ] Report Standard v2 DoD→Evidence Matrix kitöltve path+line bizonyítékkal.

## Task-specific gates

- [ ] Overlapos 2-critical konkáv pár CDE-clear interlockba oldódik; rotation nem fix listán.
- [ ] Szintetikus 3-critical: CDE-clear VAGY dokumentált `separation_fail_reason` mérhető iterációkkal
      (nem azonnali `seed_not_clear` feladás, mint a Q53C).
- [ ] `VRS_SHEET_BUILDER_SKELETON` default off → byte-azonos; budget nem starve-olja a fallbackot.
